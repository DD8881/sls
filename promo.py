...
def build_captions(pct: list, base: str, reel: bool = True) -> dict:
    ...
    # Threads: ліміт 500 симв./пост — беремо топ-3 і, про всяк випадок, ще
    # обрізаємо, якщо довгі назви штовхнули за поріг.
    n = 3
    while n >= 1:
        threads_p2 = "\n".join([
            "🔥 Топ знижки сьогодні:", *lines[:n],
            "… +ще сотні знижок у боті", "", f"📲 Усі знижки: {BOT_LINK}",
        ])
        if len(threads_p2) <= 490:
            break
        n -= 1
    tt_text = "\n".join([
